...
\begin{figure}
   ...
   \caption{The curves represent the potential of simple harmonic oscillators
            with a high frequency (left) and low frequency (right). At a given
            point, the force is the slope of the tangent to the potential. The
            integration error is indicated for both high frequency and low
            frequency motions.}
...
\begin{figure}
   ...
   \caption{The exact potential energy surface for H\sub{2} \cite{Kolos1964}
            plotted alongside the best-fitting quadratic, cubic, and quartic
            polynomials and the best-fitting Morse potential (Eq.
...
their equilibrium values, the difference between the Morse potential a quadratic
or quartic polynomial is minimal. \cite{Cramer_Book_EssentialsCompChem_2004}

\begin{equation}
   U(\vec{x}) = D_{\vec{x}} \left [ 1 - \exp \left ( \alpha _ {\vec{x}} (
                \vec{x} - \vec{x_{eq}} ) \right ) ^ 2 \right ]
...
distance between them becomes infinite, Eq. \ref{eq1:ElectrostaticInteractions}
decays so slowly that $\sum _ {i=1}^{\infty} 1 / i = \infty$. Therefore,
electrostatic interactions typically have to be evaluated over a very long
distance (or calculated completely).
...
van der Waals potential is composed of two parts---a strongly repulsive term at
interatomic distances corresponding to the overlap of van der Waals spheres of
the two atoms and an attractive term accounting for dispersion interactions. The
most common functional form used to model van der Waals interactions is called
the \emph{Lennard-Jones} (LJ) potential, shown in Eq. \ref{eq1:LennardJones}.
