...
\emph{ACE-X-NME}, where \emph{ACE} is a neutral acetyl capping residue, \emph{X}
...
\begin{table}
   ...
   $pK _ {a} ^ {calc}$ are pK\sub{a}s calculated with the proposed method using
...
The resulting structure was then used in simulations at six different solution
pH values---7.1, 8.1, 8.3, 8.7, 8.9, and 9.9. These pH values were chosen
because the pK\sub{a} of the cysteine model compound is 8.5, so the two
...
is expected to experience a negative pK\sub{a} shift with respect to the model
compound due to the electrostatic pressure of the positively-charged terminus.
Cys-4, on the other hand, is expected to experience a pK\sub{a} shift in the
...
simulations revealed pK\sub{a} shifts of -0.2 pK units for Cys 2 and +0.9 pK
...
almost identical pK\sub{a}s---8.1 and 9.4, respectively.
...
The difference in conformation sampling ability of the proposed method and the
...
Therefore, we added 21 ions---15 chloride and 6 sodium---to add ionic strength
...
calculated pK\sub{a} is shown in Table \ref{tbl4:rnase_a_ions_pkas}.
\begin{table}
...
When we added explicit ions to the simulation cell, the calculated pK\sub{a}s of
