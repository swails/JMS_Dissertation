...
residue pair.  About 40,000 full cycles are attempted each time protonation
state changes are attempted. \cite{Baptista_JPhysChemB_2001_v105_p293}
Afterwards, the solute is held fixed while MD is propagated on the solvent to
reorganize the solvent distribution to the new set of protonation states.
...
Swails_JChemTheoryComput_2012_v8_p4393} This chapter is organized as follows: I
...
\begin{figure}
   ...
   \caption[Workflow of the proposed discrete protonation CpHMD method in
            explicit solvent.]
           {Workflow of the proposed discrete protonation CpHMD method in
...
\begin{table}
   \caption[Model compound pK\sub{a} values and reference energies.]
           {Model compound pK\sub{a} values and reference energies. Because only
            differences in reference energies are used in the MC calculation,
            one state is arbitrarily assigned a reference energy of 0 (the
            deprotonated states of AS4 and GL4, the protonated state of CYS, and
            the double-protonated state of HIP). The listed reference energies
            are calculated with respect to the arbitrary zero value.  $pK _ {a}
            ^ {calc}$ are pK\sub{a}s calculated with the proposed method using
            the GB reference energy ($\Delta G _ {ref} ^ {GB}$).  Adjusted
            reference energies for explicit solvent CpHMD are labeled as $\Delta
...
\begin{figure}
   ...
   \caption[Radial distribution functions (RDFs) of solvent oxygen atoms (O) and
            hydrogen atoms (H) with different unit cell sizes.]
           {Radial distribution functions (RDFs) of solvent oxygen atoms (O) and
...
\begin{figure}
   ...
   \caption[The relaxation of the protonated state---starting from the
            ...
            function shown in purple.]
           {The relaxation of the protonated state---starting from the
            protonated trajectory---is shown in blue with its autocorrelation
...
2000 steps of dynamics with a 2 fs time step---adds dramatically to the cost of
CpHMD simulations in explicit solvent, I explored the approximation of using a
significantly smaller value for $\tau _ {rlx}$.
...
\begin{figure}
   ...
   \caption[RDFs of water oxygen atoms (O) and hydrogen atoms (H) around the
            center-of-mass of the carboxylate group]
           {RDFs of water oxygen atoms (O) and hydrogen atoms (H) around the
...
\begin{figure}
   ...
   \caption[Titration curves of Cys 2 and Cys 4 in the \emph{ACFCA}
            ...
            pH-REMD are shown]
           {Titration curves of Cys 2 and Cys 4 in the \emph{ACFCA}
            pentapeptide. Results from CpHMD (no replica exchange attempts) and
...
\begin{table}
   \caption[Calculated pK\sub{a}s for acid-range titratable residues in HEWL
            ...
            and 4LYT---without ions.]
           {Calculated pK\sub{a}s for acid-range titratable residues in HEWL
            using the proposed method for both starting structures---PDBs 1AKI
...
\begin{figure}
   ...
   \caption[RMSD plots over 20 ns of pH-REMD simulation for HEWL at pH 2, 4, 6
            and 8 with respect to the starting crystal structure 1AKI.]
           {RMSD plots over 20 ns of pH-REMD simulation for HEWL at pH 2, 4, 6
...
\begin{table}
   \caption[Calculated pK\sub{a}s for acid-range titratable residues in HEWL
            using the proposed method for both starting structures---PDBs 1AKI
            and 4LYT---with 21 ions.]
           {Calculated pK\sub{a}s for acid-range titratable residues in HEWL
            using the proposed method for both starting structures---PDBs 1AKI
            and 4LYT---with 21 ions. The root mean square error (RMSE) and
...
\begin{figure}
   ...
   \caption[Distance distribution functions calculated from HEWL simulations
            ...
            at pH 1.0.]
           {Distance distribution functions calculated from HEWL simulations
            begun with crystal structure 1AKI for all snapshots in the ensemble
            at pH 1.0. The probability distributions were calculated using
            10,000 snapshots and smoothed using a gaussian kernel density
...
\begin{table}
   \caption[Calculated pK\sub{a}s for RNase A using simulations begun from
            crystal structures 1KF5 and 7RSA.]
           {Calculated pK\sub{a}s for RNase A using simulations begun from
...
\begin{table}
   \caption[Calculated pK\sub{a}s for RNase A simulations run with explicit
            counterions present.]
           {Calculated pK\sub{a}s for RNase A simulations run with explicit
